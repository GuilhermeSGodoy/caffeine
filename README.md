...
- 🟢 Subagentes dedicados de teste (`test-writer`/`targeted-test-runner`, em `.claude/agents/`) integrados ao `finish-feature`, e gate de cobertura mínima (80% linha/branch) no CI para `Caffeine.Domain` ([#36](https://github.com/GuilhermeSGodoy/caffeine/issues/36))
  - `targeted-test-runner` executa specs/filtros indicados e resume o resultado; `test-writer` revisa o diff de uma branch em busca de lacunas de teste, sem substituir o teste principal escrito durante a implementação.
- 🟢 Bug `NG0205` (injector destruído) só ao rodar a suíte completa do frontend com cobertura habilitada ([#37](https://github.com/GuilhermeSGodoy/caffeine/issues/37))
  - Causa raiz: `EditorSessionStore.saveRequested` (auto-save debounced com RxJS, 3s) nunca cancelava sua subscription ao ser destruído. Em produção o serviço nunca é destruído (`providedIn: 'root'` vive pela sessão inteira do app), mas em teste o `TestBed` destrói e recria o serviço a cada `beforeEach` — o timer real do debounce sobrevivia a essa destruição e, ao disparar depois, chamava a API através de um injector já destruído. Corrigido implementando `ngOnDestroy` para cancelar a subscription. Com isso, o gate de cobertura mínima do frontend (80% statements/lines, 70% branches, 65% functions) também foi ativado no CI.
- 🟢 Refatoração dos subagentes de tooling: `test-runner` renomeado para `targeted-test-runner`, checkpoint de confirmação antes do `code-review --fix` no `finish-feature`, e dois agentes novos — `ci-failure-diagnostician` (acompanha o CI após abrir o PR e diagnostica falha de check, se houver) e `ef-migration-checker` (verifica migration EF Core faltando) ([#42](https://github.com/GuilhermeSGodoy/caffeine/issues/42))
...
O CI (`.github/workflows/ci.yml`) aplica um gate de cobertura mínima sobre `Caffeine.Domain` (80% linha/branch) — a única camada do backend com teste unitário puro por convenção (ver `CLAUDE.md`); `Caffeine.Infrastructure` fica fora por não ter teste de integração hoje — e sobre o frontend (80% statements/lines, 70% branches, 65% functions, via `npx ng test --configuration=ci`).
