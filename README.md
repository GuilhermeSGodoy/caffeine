...
- 🟢 Refatoração dos subagentes de tooling: `test-runner` renomeado para `targeted-test-runner`, checkpoint de confirmação antes do `code-review --fix` no `finish-feature`, e dois agentes novos — `ci-failure-diagnostician` (diagnostica falha de check do CI após abrir PR) e `ef-migration-checker` (verifica migration EF Core faltando) ([#42](https://github.com/GuilhermeSGodoy/caffeine/issues/42))
