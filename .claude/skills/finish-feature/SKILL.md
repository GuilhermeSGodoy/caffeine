...
2. **Valide localmente antes de abrir o PR** — apenas builds (rápidos, pegam erro de compilação) e os testes relevantes à mudança feita nesta branch. A suíte completa (unitários + E2E) não é mais exigida localmente — o CI (`.github/workflows/ci.yml`) é a fonte de verdade para isso. Rode `./scripts/validate-build.ps1` (nesta skill) para os builds de backend e frontend — não precisa de subagente. Para os testes, use os subagentes dedicados em vez de rodar/escrever manualmente:
...
3. **Rode a revisão de código interna do Claude Code e aplique os ajustes.** Invoque a skill `code-review` sobre o diff desta branch (`git diff main...HEAD`) no nível de esforço `medium` (equilíbrio entre pegar bugs reais e não gerar ruído — `low` pode deixar passar algo, `high`/`max` inclui achados incertos demais para aplicar sem revisão humana) com a flag `--fix`, para que os achados (bugs de correção, simplificação, reuso, eficiência) já sejam aplicados ao working tree.
   - **Revalide depois de aplicar**: os ajustes da revisão podem alterar código-fonte, então rode `./scripts/validate-build.ps1` de novo e, se os arquivos alterados tiverem specs correspondentes, invoque o **`test-runner`** de novo nesses specs antes de seguir.
...
7. **Abra o Pull Request** para `main`, usando a estrutura de `.github/PULL_REQUEST_TEMPLATE.md`, preenchendo "O que foi feito" e "Como validar", e referenciando a issue com `Closes #<numero>`.
   - `gh` (GitHub CLI) está instalado nesta máquina. Use `gh pr create --repo <owner>/<repo> --title "..." --body "..."`. Se a autenticação falhar, rode `../start-feature/scripts/gh-auth-fallback.ps1` (mesmo script usado por `start-feature`) — não peça um token ao usuário antes de tentar essa via.
